...
namespace RTMLToolKit
{
    ...
    public class RTMLCore : MonoBehaviour
    {
        ...
        [Tooltip("Number of floats that make up one frame in a DTW sequence. For pose gestures with 17 x/y keypoints, use 34.")]
        public int dtwFrameSize = 1;

        [Tooltip("Similarity threshold for DTW matching. Higher values require a closer match.")]
        public float dtwSimilarityThreshold = 0.8f;
        ...
        private void InitialiseModel()
        {
            switch (modelType)
            {
                ...
                case ModelType.KNN:
                    ...
                    break;
                case ModelType.DTW:
                    var dtw = new DTWRecognizer(inputSize, outputSize, dtwFrameSize);
        ...
        private void LoadDTW(string path)
        {
            ...
            int loadedFrameSize = data.frameSize > 0 ? data.frameSize : dtwFrameSize;
            var dtw = new DTWRecognizer(data.inputSize, data.outputSize, loadedFrameSize);
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (core.modelType == ModelType.DTW)
            {
                if (core.model is DTWRecognizer dtwModel)
                {
                    ...
                    if (dtwModel.frameSize != core.dtwFrameSize)
                    {
                        dtwModel.frameSize = Mathf.Max(1, core.dtwFrameSize);
                    }
                }
            }
